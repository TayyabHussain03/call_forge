...
_RESPONSE_KEYS: dict[ContactChannel, dict[ClarificationStage, str]] = {
    ...
    ContactChannel.UNKNOWN: {
        ClarificationStage.REPEAT: "ask_contact_channel",
        ClarificationStage.SPELL: "ask_contact_channel",
    },
}
...
class ClarificationDecision:
    ...
    Attributes:
        ...
        action: For RETRY → CLARIFY_CONTACT; for CLEAR → CONFIRM_CONTACT; for
...
class ClarificationEngine:
    ...
    def _max_for(self, channel: ContactChannel) -> int:
        ...
        if channel in (ContactChannel.PHONE, ContactChannel.WHATSAPP, ContactChannel.SMS):
            return int(self._config.limits.get("phone_clarification_max_attempts", 3))
        if channel == ContactChannel.UNKNOWN:
            # Ambiguous channel — "kaunsa method?" poochne ke liye bounded retries.
            return int(self._config.limits.get("email_clarification_max_attempts", 3))
    ...
    def evaluate(
        ...
        Logic:
            - CLEAR: extraction_clear True → outcome CLEAR, stage NONE (clean
              reset), attempts unchanged (increment NAHI), action CONFIRM_CONTACT.
